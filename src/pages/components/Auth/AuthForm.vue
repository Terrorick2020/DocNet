<template>
    <div class="main__form">
        <h1 class="form__title">{{ `${ isReg ? 'Регистрация' : 'Авоторизация' }` }}</h1>
        <form>
            <div class="form__item">
                <label for="username" :style="`top: ${ isFocusUsername || username ? '-20px' : '0px' }`">Имя</label>
                <input type="text" id="username" v-model="username" @focus="upUsernameInput" @blur="downUsernameInput">
            </div>
            <div class="form__item">
                <label for="password" :style="`top: ${ isFocusPassword || password ? '-20px' : '0px' }`">Пароль</label>
                <input type="text" id="password" v-model="password" @focus="upPasswordInput" @blur="downPasswordInput">
            </div>
            <div class="form__info">
                <label>{{ `${ isReg ? 'Уже зарегистрированы?' : 'Еще нет аккаунта?' }` }}</label>
